...
public final class GsalesDao
{
	...
	public ArrayList<Gsales> dailyGsales()
	{
		...
		String sql = "select gname,gprice,gnum, allSum from goods, (select gid as salesid,sum(snum) as allSum from gsales where to_days(sdate)=to_days(now()) group by gid) as temp where gid = 1"; 
		try
		{
	 ...
	 *@return boolean
	 */
	public boolean shoppingSettlement(Gsales gSales,String date)
	{
		boolean bool = false;
		conn = DbConn.getconn();
